...
Agent-oriented adverbs are fine with transitive verbs~(\nextx a) but not with unaccusatives~(\nextx b).
 ...
 \begin{xlist} 
 	\ex {   
 		\gll  teo \glemph{taraf} et ha-laxmanja \underline{be-raavtanut}\\
 		  Theo devoured.\gsc{SMPL} \gsc{ACC} the-bread.roll.\gsc{F} in-voracity\\
 		\glt `Theo devoured the roll ravenously.' } 
		
 	\ex   [*]{  
 		\gll  ha-bakbuk \glemph{nafal} \underline{be-mejomanut}\\
...
The Hebrew equivalent of `by itself', \emph{me-a{\ts}mo} (lit.~`from himself/itself'), diagnoses the non-existence of an external argument. The phrase is not compatible with direct objects of transitive verbs~(\nextx a) but is valid with unaccusatives~(\nextx b).
 ...
 \begin{xlist} 
 	...
 	\ex {   
 		\gll  ha-bakbuk \glemph{nafal} \underline{me-a{\ts}mo}\\
...
\textbf{The first test} is the ordering of the subject and the verb. Modern Hebrew is typically SV(O), but promoted subjects may appear after the verb, resulting in VS order. This is true for both unaccusatives and passives, presumably because the underlying object remains in its original vP-internal position. Unergatives do not allow VS, with the exception of a marked structure referred to as ''stylistic inversion''. For additional discussion see \cite{shlonsky87}, to whom the test is attributed, as well as \cite{shlonskydoron91}, \cite{borer95} and \cite{preminger10} for other aspects. Transitive configurations are only possible in this kind of inversion~(\nextx a), whereas unaccusative verbs are unmarked~(\nextx b).
 ...
 \begin{xlist} 
 	\ex    [\#]{ 
 	\gll  \glemph{kaf{\ts}-u} \underline{ʃloʃa} \underline{klavim} be-ʃmone ba-boker\\
 	  jumped.\gsc{SMPL}-\gsc{3PL} three dogs in-eight in.the-morning\\
 	\glt `And thence jumped three dogs at 8am.' (Marked variant) } 
	
 	\ex {    
 	\gll  \glemph{nafl-u} \underline{ʃaloʃ} \underline{kosot} be-ʃmone ba-boker\\
...
A transitive construction is compatible with the possessive dative~(\nextx a), as is a non-active construction~(\nextx b), whereas an unergative verb leads to an affected interpretation of the kind discussed by \cite{arieletal15} and \cite{barashersiegalboneh16}, (\nextx c). 
 ...
 \begin{xlist} 
 	\ex {   
 	\gll  dana \glemph{ʃavr-a} \underline{l-i} et ha-ʃaon\\
 	  Dana broke.\gsc{SMPL}-\gsc{F.SG} to-me \gsc{ACC} the-watch\\
 	\glt `Dana broke my watch.' } 
	
	
 	\ex {   
 		\gll  ha-maftexot \glemph{nafl-u} \underline{l-i}\\
 		  the-keys fell.\gsc{SMPL}-\gsc{PL} to-me\\
 		\glt `My keys fell.' } 
	

 	\ex   [\#]{  
 		\gll  ha-klavim \glemph{kaf{\ts}-u} \underline{l-i}\\
...
Typical change of state predicates can also be found as unaccusatives in this template:
 ...
 \begin{xlist} 
 	...
 	\ex {   
 		\gll  \glemph{kafa} \underline{le-teo} ha-bakbuk\\
...
\hammer{
 \begin{exe}
 \ex  \label{ex:gen-tkal}\glemph{Generalizations about {\tkal}} 
 \begin{xlist} 
 	\ex  \glemph{Configurations} Verbs appear in all possible argument structure configurations. 
 	\ex  \glemph{Alternations:} Participates in alternations with the other templates, as will be reviewed throughout the book. 
 \z
\z 
}
...
Assuming that the active variant is the elsewhere case, certain roots will be said to require a non-active alloseme of Voice~(\nextx a) and others will be compatible with agentive verbs~(\nextx b):\footnote{Chapter~\ref{chap:aas} contains a brief comparison of contextual allosemy with one alternative, namely postulating homophonous heads.}
 ...
 \ex  \denote{Voice} =  
 \begin{xlist} 
 	\ex  $\lambda$P.P \phantom{agent(x,e)xxx} / \trace~ \{ \root{npl} `\root{\gsc{FALL}}', \root{kpa} `\root{\gsc{FREEZE}}' , \dots \} 
...
\begin{small}
 ...
 \lsptoprule
		...
		1\gsc{SG} & \multicolumn{2}{c}{XaYaZ-ti} & XoYeZ & XoYeZ-et & \multicolumn{2}{c}{e-XYoZ/ji-XYoZ}\\\midrule
		1\gsc{PL} & \multicolumn{2}{c}{XaYaZ-nu} & XoYZ-im & XoYZ-ot & \multicolumn{2}{c}{ni-XYoZ}  \\\midrule\hline
		2\gsc{SG} & XaYaZ-ta & XaYaZ-t & XoYeZ & XoYeZ-et & ti-XYoZ & ti-XYeZ-i\\\midrule
		2\gsc{PL} & XaYaZ-tem & XaYaZ-ten/tem & XoYZ-im & XoYZ-ot & \multicolumn{2}{c}{ti-XYeZ-u}\\\midrule\hline
		3\gsc{SG} & XaYaZ & XaYZ-a & XoYeZ & XoYeZ-et & ji-XYoZ & ti-XYoZ\\\midrule
		3\gsc{PL} & \multicolumn{2}{c}{XaYZ-u} & XoYZ-im & XoYZ-ot & \multicolumn{2}{c}{ji-XYeZ-u}\\\midrule
\lspbottomrule
 	\end{tabularx}
\end{small}
}
...
To understand the syntax-semantics of {\tpie}, consider the pairs in~(\ref{ex:va-piel1}). In~(\nextx a), both Agents and Causes are possible. In~(\ref{ex:va-piel1}b) only the Agent is possible. The (a) example has the verb in {\tkal}, the (b) example in {\tpie}.
 ...
 \begin{xlist} 
 	\ex {   
 		\gll  \emph{\{}\cmark~ha-jeladim / \cmark~ha-tiltulim ba-argaz\emph{\}} \glemph{ʃavr}-u et ha-kosot\\
 		  \phantom{\{\cmark~}the-children {} \phantom{\cmark~}the-shaking in.the-box \glemph{broke.\gsc{SMPL}}-\gsc{PL} \gsc{ACC} the-glasses\\
 		\glt `\{The children / Shaking around in the box\} broke the glasses.' } 
		
	
 	\ex {   
 		\gll  \emph{\{}\cmark~ha-jeladim / \xmark~ha-tiltulim ba-argaz\emph{\}} \glemph{ʃibr}-u et ha-kosot\\
 		  \phantom{\{\cmark~}the-children {} \phantom{\xmark~}the-shaking in.the-box \glemph{broke.\gsc{INTNS}}-\gsc{PL} \gsc{ACC} the-glasses\\
...
\ex { \label{ex:voice:piel-meanings}Pretheoretical classification of some verbs in \tpie: 
 ...
 \lsptoprule
	...
	\multirow{3}{*}{Intensive} & a.& \root{ʃbr} & ʃavar & `broke' & ʃiber & `broke to pieces'\\
	    ...
	    & c.& \root{'kl} & axal & `ate' & ikel & `corroded, consumed'\\\midrule

 	\multirow{3}{*}{Pluractional} & d.& \root{hlx} & halax & `walked' & hilex & `walked around'\\
 	    & e.& \root{r\dgs{k}d} & rakad & `danced' & riked & `danced around'\\
  	    & f.& \root{\dgs{k}fts} & kafats & `jumped' & kipets/kiftsets & `jumped around'\\\midrule
...
In all cases, the verbs are active: either unergative or transitive. And in all cases, the external argument is agentive. In some examples this contrast is clear: a storm cannot ``intensively'' break a window to bits.
 ...
 \begin{xlist} 
 	...
 	\ex {   
 		\gll  ha-jeladim \glemph{ʃivr-u} et ha-xalon le-xatixot \underline{be-xavana}\\
 		  the-children broke.\gsc{INTNS}-\gsc{PL} the-window to-pieces in-purpose\\
 		\glt `The children broke the window to bits on purpose.' } 
		
 	\ex {   
 		\gll \ljudge{*}ha-sufa \glemph{ʃibr-a} et ha-xalon (le-xatixot)\\
...
\hammer{
 \begin{exe}
 \ex  \label{ex:gen-tpie}\glemph{Generalizations about {\tpie}} 
 \begin{xlist} 
 	\ex  \glemph{Configurations:} Verbs appear in active (transitive/unergative) configurations. 
		Readings are weakly agentive.
 	\ex  \glemph{Alternations:} When alternating with {\tkal}, provides a more ``intensive'' or agentive version. 
 \z
\z 
}
...
A number of recent works on argument and event structure have identified a component of meaning that can be broadly described as agentive, volitional, or a ``direct cause''. The most straightforward view of agentivity equates it with animacy. For example, Italian \emph{fare}-causatives require the causee to be animate, as in~(\nextx). Similar considerations are familiar from control phenomena as discussed in a range of work from \cite{farkas88} to \cite{zu18phd}.
 ...
 \begin{xlist} 
 	...
 	\ex {   
 		\gll  \ljudge{\#}Gianni ha fatto rompere la finestra al ramo\\
...
For example, in Italian causatives without \emph{fare}, inanimate causers vary with respect to how acceptable they are. A branch is fine, but a storm is not. The explanation is that the branch is a direct causer but the storm is not a proximate enough cause; it is not teleologically capable.\footnote{See \cite{irwin19tlr} for an explication of some teleological properties in terms of body parts.}
 ...
 \begin{xlist} 
 	...
 	\ex {   
 		\gll  \ljudge{?}Il vento ha rotto la finestra\\
 		  the wind has broken the window\\
 		\glt `The wind broke the window.' } 
		
 	\ex {   
 		\gll  \ljudge{\#}Il temporale ha rotto la finestra\\
 		  the storm has broken the window\\
 		\glt (int.~`The storm broke the window') \hfill (Italian, \citealt[195]{folliharley08)} } 
...
Given its meaning and its similarity to an analytic paraphrase, (\nextx), \cite{spathasetal15} propose the denotation in~(\anextx).
 ...
 \begin{xlist} 
 	...
 	\ex {   
 		...
 		\glt `John accuses himself.'  \hfill \citep[63--64]{alexiadouafto} } 
...
In order to explain the behavior of verbs in {\tpie} I propose to use a special root {\va}, which enforces agentive (or weakly agentive) readings.\footnote{\cite{doron03} uses a syntactic head $\iota$; see Chapter \ref{vz:others:ed} on some differences between the theories.} I assume that {\va} attaches to the verbal spine at the vP level, thereby triggering the agentive alloseme of Voice (following \citealt{doron03,doron14adj}). The morphophonology produces the templates {\tpie} and {\thit}, as I return to momentarily. Here is the basic proposal, followed by a deep dive into each part (syntax, semantics and phonology).
 ...
 \begin{xlist} 
 	...
 	\ex {  \denote{Voice} = $\lambda$x$\lambda$e.Agent(x,e) / \trace~\va 
...
The derivation of verbs in {\tpie} which do not alternate with {\tkal} is identical. For (\nextx), we first build up the core vP, then attach {\va}, and then attach the external argument. The fact that the core vP cannot combine with Voice directly must be listed with the root, in whatever way regulates which functional heads can appear with which root. Now the meaning of the root is chosen by {\va}, rather than by v (since there is no verb in {\tkal}) or Voice (since {\va} is closer to the root, \citealt{arad03,marantz13,elenasamioti14}), however {\va} is licensed by the root formally.
 ...
 \begin{xlist} 
    ...
    \ex {   
     \gll  \underline{ha-xom} \glemph{ʃibeʃ} et ha-medidot\\
...
Given that {\va} has been just argued to be a root, assigning a semantics to it without a theory of root semantics is difficult. What we can do is see its effects on the external argument, formalized as follows:
 ...
 \begin{xlist} 
 	\ex {  $\lambda$P.P \phantom{agent(x,e)xxx} / \trace~ \{ \root{npl} `\root{\gsc{FALL}}', \root{kpa} `\root{\gsc{FREEZE}}' , \dots \} 
 	\ex {  $\lambda$x$\lambda$e.Agent(x,e) or $\lambda$x$\lambda$e.Cause(x,e) 
 	\ex {  $\lambda$x$\lambda$e.Agent(x,e) / \trace~\va 
 \z
\z 
...
In the examples in~(\lastx) the event does not entail change of state, unlike with breaking and eating/corroding. So perhaps there is a tripartite division of roots to be made, as follows:
 ...
 \ex  
 \begin{xlist} 
 	\ex {  \glemph{Other-oriented roots (change of state)} such as \root{\gsc{BREAK}} and \root{\gsc{PRODUCE}}: pluralization of the object. 
 	\ex {  \glemph{Activity roots} or \textbf{self-oriented roots} such as \root{\gsc{RUN}} and \root{\gsc{JUMP}}: pluralization of the spatio-temporal aspects of the event. 
 	\ex {  \glemph{Other cases:} no pluralization. 
 \z
\z 
...
A number of verbs in {\tpie} stretch the notion of ``agentivity'' to the point where even a weak definition is no longer tenable. In the examples in~(\nextx), the verb can hardly be described as agentive since the subject is inanimate, while in~(\anextx) the subject is animate but non-volitional. These verbs are compatible with agentive subjects as well, but clearly do not require them.
 ...
 \begin{xlist} 
   \ex {   
     \gll  \underline{ha-midgam} \glemph{ʃikef} et totsot ha-emet\\
       the-poll reflected.\gsc{INTNS} \gsc{ACC} results.\gsc{CS} the-truth\\
     \glt `The polls (correctly) reflected the results.' } 
  
    
   \ex {   
     \gll  be-ritsa axat \underline{ha-ʃaon} ʃel garmin kimat \glemph{diek} kaaʃer hetsig stia kimat xasrat maʃmaut ʃel axuz ve-ktsat\\
...
\z 
 ...
 \begin{exe}
\ex {   
   \gll  \underline{ha-kadurselan-it} \glemph{kibl-a} maka xazaka ba-regel\\
...
\begin{small}
 ...
 \lsptoprule
		& \multicolumn{2}{c}{Past} & \multicolumn{2}{c}{Present} &  \multicolumn{2}{c}{Future} \\\midrule
		& \gsc{M} & \gsc{F} & \gsc{M} & \gsc{F} & \gsc{M} & \gsc{F} \\\midrule\hline
		1\gsc{SG} & \multicolumn{2}{c}{Xi\dgs{Y}aZ-ti} & me-Xa\dgs{Y}eZ & me-Xa\dgs{Y}eZ-et & \multicolumn{2}{c}{a-Xa\dgs{Y}eZ/je-Xa\dgs{Y}eZ}\\\midrule
		1\gsc{PL} & \multicolumn{2}{c}{Xi\dgs{Y}aZ-nu} & me-Xa\dgs{Y}Z-im & me-Xa\dgs{Y}Z-ot & \multicolumn{2}{c}{ne-Xa\dgs{Y}eZ}  \\\midrule\hline
		2\gsc{SG} & Xi\dgs{Y}aZ-ta & Xi\dgs{Y}aZ-t & me-Xa\dgs{Y}eZ & me-Xa\dgs{Y}eZ-et & te-Xa\dgs{Y}eZ & te-Xa\dgs{Y}Z-i\\\midrule
		2\gsc{PL} & Xi\dgs{Y}aZ-tem & Xi\dgs{Y}aZ-ten/m & me-Xa\dgs{Y}Z-im & me-Xa\dgs{Y}Z-ot & \multicolumn{2}{c}{te-Xa\dgs{Y}Z-u}\\\midrule\hline
		3\gsc{SG} & Xi\dgs{Y}eZ & Xi\dgs{Y}Z-a & me-Xa\dgs{Y}eZ & me-Xa\dgs{Y}eZ-et & je-Xa\dgs{Y}eZ & te-Xa\dgs{Y}eZ\\\midrule
		3\gsc{PL} & \multicolumn{2}{c}{Xi\dgs{Y}Z-u} & me-Xa\dgs{Y}Z-im & me-Xa\dgs{Y}Z-ot & \multicolumn{2}{c}{je-Xa\dgs{Y}Z-u}\\\midrule
\lspbottomrule
 	\end{tabularx}
\end{small}
}
...
\hammer{
 \begin{exe}
 \ex  \label{ex:gen-tkal2}\glemph{Generalizations about {\tkal}} 
 \begin{xlist} 
 	\ex  \glemph{Configurations:} Verbs appear in all possible argument structure configurations. 
 	\ex  \glemph{Alternations:} Participates in alternations with the other templates, as will be reviewed throughout the book. 
 \z
...
\hammer{
 \begin{exe}
 \ex  \label{ex:gen-tpie2}\glemph{Generalizations about {\tpie}} 
 \begin{xlist} 
 	\ex \glemph{Configurations:} Verbs appear in active (transitive/unergative) configurations. 
		Readings are weakly agentive.
 	\ex  \glemph{Alternations:} When alternating with {\tkal}, provides a more ``intensive'' or agentive version. 
