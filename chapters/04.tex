...
\raisebox{-2.5em}{ \begin{tabular}{l|l|ll|ll}
	...
	a.& \root{'xl} & \emph{axal} & `ate'  & \emph{heexil (be-)} & `fed (with)'\\
...
Using {\thit} as the Vocabulary Item spelling out {\vd} is shorthand for a more detailed morphophonology. A sample derivation is adapted here from~\cite{kastner18nllt}. I contrast the \gsc{3SG.M.Past} form with the \gsc{1SG.Past} form, which has an affix and a different stem vowel.
\pex
...
\pex
	\a \emph{niʃbar} `was broken'\\
		Breaking event of a DP, no cause specified.\footnote{This form is still compatible with a passive reading as well, with an implicit agent.}
	\a \emph{ʃavar} `broke'\\
		Breaking event of a DP, external argument specified in the syntax and understood as agent.
\xe
...
In terms of syntax, there appears to be no difference between the constructions. Both are monoeventive and can only be modified with one temporal adverb, and do not differ in the readings of `again' \citep{vonstechow96}.
@@run diagnostics@@
\pex 
	\a \begingl
...
As a result, \citet[62]{doron03} must conclude that ``\emph{x reddened} is equivalent to \emph{Something caused x to redden}'', with the Causative head \gsc{CAUS} introducing a Causer that is existentially quantified over. This kind of account is more in line with a passive analysis than a causative one.\footnote{Although Doron's morphosemantic head formed the direct inspiration for the current {\vd}, just as her \gsc{MIDDLE} and \gsc{INTNS} heads paved the way for {\vz} and {\va}.}
