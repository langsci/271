...
This chapter, the longest of the book, introduces a number of constructions across two different templates. The names I have given these constructions are intended to be transparent and easy to compare with work on other languages. With that in mind, the richness of the system could also be confusing. What is important in terms of the big picture is that the two kinds of vPs discussed so far (one with {\va} and one without it) can merge with the non-active head {\vz}, and not just with regular Voice as in the previous chapter. In addition, there is a prepositional counterpart to this head, namely {\vz}, which derives another kind of construction -- the figure reflexive. And finally, pure reflexives are only possible when {\va} is in the structure. The table in~(\nextx) provides a preview.
...
\ex \begin{tabular}{c|cc}
	Construction	& {\tnif}	& {\thit} \\\hline
Anticausative	& {\vz}	& {\va}, {\vz}\\
Inchoative & {\vz}	& {\va}, {\vz}\\
Figure reflexive	& {\pz}	& {\va}, {\pz}\\
Reflexive	& ---	& {\va}, {\vz}\\
\end{tabular}
\xe

These constructions are explored as follows. In Section~\ref{vz:tnif} I explore the left-hand side of the table, identifying the anticausatives, inchoatives and figure reflexives of {\tnif} (this last group underwriting a novel generalization). Section~\ref{vz:vz} analyzes the first two and Section~\ref{vz:pz} analyzes figure reflexives. Section~\ref{vz:interim} briefly summarizes the picture for {\tnif}. I then move to the right-hand side of the table, to {\thit}, in Section~\ref{vz:thit}. Anticausatives, inchoatives and reflexives are analyzed in Section~\ref{vz:vzva}; figure reflexives are discussed in Section~\ref{vz:pzva}. Section~\ref{vz:others} then compares the current approaches with other treatments in the literature; Section~\ref{vz:conc} summarizes and concludes.
...
\section{Interim summary} \label{vz:interim}
