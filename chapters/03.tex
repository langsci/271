...
The current chapter motivates the non-active head {\vz}. Informally, {\vz} rules out the addition of an external argument. In the simplest \isi{case}, this configuration leads to argument structure alternations as in Table~\ref{tab:3-1:anticaus}, where the anticausative variants are essentially marked\is{markedness} with non-active morphology. The two templates explored in this chapter are {\tnif} and {\thit}, on the right-hand side of each row in the table.
\begin{table}
...
Anticausative and inchoative verbs in {\tnif} are compatible with the possessive dative\is{unaccusativity tests}\is{\isi{case}}, again because it presumably targets the internal argument.
 \begin{exe}
 \ex  
 \begin{xlist} 
 	\ex  	 
...
The \textbf{possessive dative\is{unaccusativity tests}\is{\isi{case}}} is likewise incompatible with \isi{figure reflexives}; example~(\nextx) is infelicitous on a reading where the cat is the speaker's.
 \begin{exe}
\ex   
...
The famous \isi{case} of \isi{deponents} in \ili{Latin} is similar: as discussed by various authors (e.g.~\citealt{xuetal07}), \isi{deponents} are verbs with nonactive morphology but active syntax. Although they appear with a nonactive suffix, the verbs themselves are unergative or \isi{transitive}. The deponent verb \emph{sequor} `to follow' is syntactically \isi{transitive} but has no morphologically active forms:
 \begin{exe}
 \ex  
 \begin{xlist} 
