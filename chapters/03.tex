...
These are the inchoatives treated here, but similar constructions can be found in Romance languages. \cite{burzio86} observes what he calls an ``inherently reflexive'' verb which requires the nonactive clitic \emph{si} (\ili{Italian} \gsc{SE}). The glosses are his.
 ...
 {\multicolsep=.5\baselineskip\columnsep=-.5cm\begin{multicols}{2}\raggedcolumns
 \begin{xlist} 
 	\ex   
...
I discuss the syntax and semantics together in what follows.
	...
	\subsection{Syntax and semantics} \label{vz:pz:syn}	
		\subsubsection{Ordinary prepositions}\largerpage
...
On \citeauthor{wood15springer}’s (\citeyear{wood15springer}) analysis, the role of \isi{Figure} is not saturated within the \emph{p}P, since no DP is possible in Spec,{\pz}P. Rather, an argument introduced later, in Spec,VoiceP, saturates this predicate. The schematic structure in \figref{fig:3n46} shows the assignment of semantic roles using dashed arrows.
 
 \begin{figure}
% \ex  
\caption{\color{red}Please insert caption\label{fig:3n46}}
		 \Tree
		 [.VoiceP
...
The structure for~(\ref{ex:vz:is-figrefl}) is given in \figref{tree:vz:is-figrefl}, adapted from \citet[170]{wood15springer}. \citeauthor{wood15springer}'s insight is that there is no argument filling Spec,{\pz}P which can saturate the \isi{Figure} role of {\pz}. The next DP merged in the structure, \emph{hann} `he', will then saturate both Voice's semantic role (\isi{Agent}) and the role of \isi{Figure} introduced by {\pz}. A variety of diagnostics for Icelandic show that the verb is agentive, with the DP \emph{Hann} merged in Spec,VoiceP, just like Hebrew \isi{figure reflexives} are agentive.
 ...
 \begin{figure}
% \ex 
\caption{\color{red}Please insert a caption\label{tree:vz:is-figrefl} }
		\Tree
		[.VoiceP
...
Continuing on to \textsc{inchoatives}, they pattern with anticausatives. They are incompatible with agent\is{Agent}-oriented adverbs and \emph{by}-phrases:
 \begin{exe}\judgewidth{??}
 \ex  
 \begin{xlist} 
 		\ex    
...
\caption{{\color{red}Please provide a caption}. †: The exact denotation of T is immaterial here.\label{tree:vz:thit-refl}}
\footnotesize
	\Tree
...
Overall, the analysis showcases how complex structure ({\vz} and\linebreak {\va}) correlates with complex meaning and complex morphology. On the meaning side of things, reflexives in Hebrew do not come from a dedicated functional or lexical item. There must be some conspiracy of factors in order to derive a reflexive reading. The complex structure is tracked by complex morphology: verbs in {\thit} have a number of distinguishing morphophonological properties, namely the prefix, the non-spirantized medial root consonant \dgs{Y}, and the stem vowels inherent to the template.
 ...
 \begin{xlist} 
 	\ex   
		\emph{kava} `set':\\
 		...
 		\pagebreak\ex  \emph{hitkabea} `was cemented': \\
				\Tree
				[.VoiceP
					[.EA ]
					[.
