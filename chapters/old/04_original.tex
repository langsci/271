...
\pex
	...
	\endgl
	\a \begingl
		\gla ana \textbf{heexil-a} et ema (uxmanjot)//
		\glb Anna fed\gsc{-F} \gsc{ACC} Emma blueberries//
		\glft `Anna fed Emma (blueberries).'//
	\endgl %Emma is a dog
\xe
...
The most common way of characterizing these alternations informally is by saying that {\thif} is a causative version of {\tkal}. Yet what we see in actuality is that verbs in {\thif} are active, regardless of whether they alternate with an unaccusative verb in {\tkal}, a transitive verb in {\tkal}, or nothing in {\tkal}. A few examples of the causative alternation in this template are given in~(\nextx a). Many verbs are also causative without alternating, as in~(\nextx b), and others are unergative,~(\nextx c).
...
\pex 
	\a \emph{Mary} \textbf{Voice} [$_{\text{vP}}$ \emph{broke the glass}].
...
\pex
	...
	\a \begingl
		\gla ima heexil-a et beki (uxmanjot)//
		\glb mom fed\gsc{-F} \gsc{ACC} Becky blueberries//
		\glft `Mom fed Becky (blueberries).'//
		\endgl\\
...
\ex \begingl
	\gla ha-malka \textbf{hexnis-a} et ha-sefer la-tik//
	\glb the-queen inserted.\gsc{CAUS-F} \gsc{ACC} the-book to.the-bag//
	\glft `The queen put the book in the bag.'//
	\endgl
\xe
...
\pex
  ...
  \a Inchoative:\\ \begingl
    ...
    \glft `The cat grew as fat as a pig / fatter than a pig.'//
  \endgl
